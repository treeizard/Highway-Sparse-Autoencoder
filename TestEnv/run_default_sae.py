...
for env_id in ENV_LIST:
    ...
    # Output dirs
    output_dir = os.path.join("data_original", model_name + "_sae", scenario_name)
    ...
    os.makedirs(os.path.join(output_dir, "latent"), exist_ok=True)  # <-- Save latents
    if record_video:
    ...
    for demo_id in trange(1, num_scenarios + 1, desc=f"{scenario_name}"):
        ...
        while not done and step < max_steps:
            ...
            with torch.no_grad():
                ...
                latent = sae.relu(sae.encoder(h2))  # No perturbation
            ...
            latent_log.append(latent.cpu().numpy())  # <-- Save latent activation
        ...
        np.save(os.path.join(output_dir, "latent", f"demo{demo_id}.npy"), np.array(latent_log))  # <-- Save latent
