...
# Switch Function
# -------------------------------
def switch_vehicle_positions(obs_matrix, i, j):
    """
    Switch the positions of vehicles at index i and j (1-based, as 0 is ego).
    """
    obs_matrix = obs_matrix.copy()
    obs_matrix[[i, j]] = obs_matrix[[j, i]]
    return obs_matrix
...
for test_name, (i, j) in test_cases.items():
    for env_id in ENV_LIST:
        scenario_name = env_id.split('-')[0]
        print(f"\n=== Running {num_scenarios} episodes on {env_id} [{test_name}] ===")

        # Output dirs
        output_dir = os.path.join("data_dyna_test", test_name, model_name + "_sae", scenario_name)
        ...
        for demo_id in trange(1, num_scenarios + 1, desc=f"{scenario_name}_{test_name}"):
            obs_log, label_log, action_log, latent_log, frame_log = [], [], [], [], []
            ...
            while not done and step < max_steps:
                obs_switched = switch_vehicle_positions(obs, i, j)
                obs_tensor = torch.tensor(obs_switched.flatten(), dtype=torch.float32, device=device).view(1, -1)
                ...
                latent_log.append(latent.cpu().numpy())
            ...
            np.save(os.path.join(output_dir, "latent", f"demo{demo_id}.npy"), np.array(latent_log))
